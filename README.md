...
* `mapSize` - This can be used to specify a fixed amount of how much virtual memory address space (in bytes) to allocate for mapping to the database files. Setting a fixed map size can improve performance of reads since it can reduce the amount of dynamic memory mapping is required. If you set this, it should be set larger than the database will ever grow to (for example, you might set this to 1TB). Different OSes have different allocation limits. If this is not set, dynamic memory mapping is used by default (`remapChunks`).
* `remapChunks` - This a flag to specify if dynamic memory mapping should be used. This is enabled by default if no `mapSize` is specified, otherwise it is disabled by default.
